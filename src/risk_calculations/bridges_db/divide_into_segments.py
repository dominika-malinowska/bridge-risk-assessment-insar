...
from ps_predictions.second_paper.bridges_db.utils.bridge_divisions import (
...
if __name__ == "__main__":
    ...
    # vulnerability_path = os.path.join(data_path, "lsb_bridges_database")

    # # Define the input shapefile path
    # # input_path_shp = os.path.join(
    # #     vulnerability_path, "lsb_OSM_poly_vuln_exp_lines_test.shp"
    # # )
    # input_path_shp_poly = os.path.join(vulnerability_path, "lsb_OSM_polygons.shp")
    # input_path_shp_lines = os.path.join(vulnerability_path, "lsb_OSM_lines.shp")

    # # Define the output shapefile path
    # output_path_shp = os.path.join(vulnerability_path, "new_centerlines_test.shp")
    
    main_path = os.path.join("/mnt/g/SOCIAL_PAPER")
    data_path = os.path.join(main_path, "HPC_output_updated")

    # Define the input shapefile path
    # input_path_shp = os.path.join(
    #     vulnerability_path, "lsb_OSM_poly_vuln_exp_lines_test.shp"
    # )
    input_path_shp_poly = os.path.join(data_path, "combined_nbi_polygons.shp")
    input_path_shp_lines = os.path.join(data_path, "combined_nbi_lines.shp")
    ...
    for index, row in gdf_poly.iterrows():
        ...
        if isinstance(polygon, Polygon):
            ...
            if centerline is None:
                print(f"bridge with ID {bridge_id} needs a different solutions, bridge length: {row['Total Leng']}")
                continue
    ...
    gdf_poly.drop("segments", axis=1).to_file(output_path_shp)
    ...
    profile_output_file = "profiling_results.prof"
    profiler.dump_stats(profile_output_file)

    # Optionally, read and print the results from the file
