...
if __name__ == "__main__":
    ...
    MAX_MONITORING_COEFFICIENT = 1.55
    ...
    bridges_db_path = os.path.join(
        data_path, "lsb_bridges_database", "LSB Database_corrected.csv"
    )
    exposure_path = os.path.join(data_path, "lsb_bridges_database", "lsb_OSM_lines.csv")
    ...
    # Write the GeoDataFrame to a new CSV file
    bridges_gdf.to_csv(
        os.path.join(data_path, f"lsb_risk_analysis_{MAX_MONITORING_COEFFICIENT}.csv"),
        index=False,
    )

    # Write the GeoDataFrame to a pickle
    bridges_gdf.to_pickle(
        os.path.join(data_path, f"lsb_risk_analysis_{MAX_MONITORING_COEFFICIENT}.pkl")
    )
    ...
    filtered_bridges_gdf.to_csv(
        os.path.join(
            data_path, f"lsb_risk_analysis_filtered_{MAX_MONITORING_COEFFICIENT}.csv"
        ),
        index=False,
    )
    ...
    filtered_bridges_gdf.to_pickle(
        os.path.join(
            data_path, f"lsb_risk_analysis_filtered_{MAX_MONITORING_COEFFICIENT}.pkl"
        )
    )
    ...
    filtered_bridges_gdf.to_file(
        os.path.join(data_path, f"lsb_risk_analysis_{MAX_MONITORING_COEFFICIENT}.shp")
    )
